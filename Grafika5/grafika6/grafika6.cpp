...
GLuint vao[5], vbo[9], ebo, tex, texw1, texw2, texw3;	// identyfikatory poszczegolnych obiektow (obiekty tablic wierzcholkow, buforow wierzcholkow, elementow, tekstury)
...
GLfloat tex_wall[] = {	//wspolrzedne tekstury dla scian
	0.0f, 0.0f,	// lewy dolny rog tekstury
	5.0f, 0.0f,
	5.0f, 5.0f,
	0.0f, 5.0f, // prawy gorny rog tekstury
...
float pixels_wall1[] = {
	1.0f, 0.0f, 0.0f,   0.1f, 0.1f, 0.1f,   0.2f, 0.2f, 0.2f,
	0.1f, 0.1f, 0.1f,   1.0f, 0.0f, 0.0f,   0.3f, 0.3f, 0.3f,
	0.2f, 0.2f, 0.2f,   0.3f, 0.3f, 0.3f,   1.0f, 0.0f, 0.0f,
};

float pixels_wall2[] = {
	0.0f, 1.0f, 0.0f,   0.1f, 0.1f, 0.1f,   0.2f, 0.2f, 0.2f,
	0.1f, 0.1f, 0.1f,   0.0f, 1.0f, 0.0f,   0.3f, 0.3f, 0.3f,
	0.2f, 0.2f, 0.2f,   0.3f, 0.3f, 0.3f,   0.0f, 1.0f, 0.0f,
};

float pixels_wall3[] = {
	0.0f, 0.0f, 1.0f,   0.5f, 0.5f, 0.1f,   0.6f, 0.6f, 0.2f,
	0.5f, 0.5f, 0.1f,   0.0f, 0.0f, 1.0f,   0.7f, 0.7f, 0.3f,
	0.6f, 0.6f, 0.2f,   0.7f, 0.7f, 0.3f,   0.0f, 0.0f, 1.0f,
};
...
void create_objects()
{
	...
	glGenBuffers(9, vbo);		// obiekty buforow wierzcholkow, dla kazdego typu atrubutow kazdego obiektu mamy jeden bufor (np. bufor dla kolorow podlogi, bufor dla wspolrzednych podlogi itd.)
	...
	glBindBuffer(GL_ARRAY_BUFFER, vbo[5]);	// bufor wspolrzednych wierzcholkow sciany1
	glBufferData(GL_ARRAY_BUFFER, sizeof(ver_wall1), ver_wall1, GL_STATIC_DRAW);
	glVertexAttribPointer(posAttrib, 3, GL_FLOAT, GL_FALSE, 0, 0);
	glEnableVertexAttribArray(posAttrib);
	/*
	glBindBuffer(GL_ARRAY_BUFFER, vbo[1]);	// bufor kolorow wierzcholkow sciana1
	glBufferData(GL_ARRAY_BUFFER, sizeof(col_floor), col_floor, GL_STATIC_DRAW);
	glVertexAttribPointer(colAttrib, 3, GL_FLOAT, GL_FALSE, 0, 0);
	glEnableVertexAttribArray(colAttrib);
	*/
	glBindBuffer(GL_ARRAY_BUFFER, vbo[8]);	// bufor wspolrzednych tekstury sciana1
	...
	glBindBuffer(GL_ARRAY_BUFFER, vbo[6]);	// bufor wspolrzednych wierzcholkow sciany1
	...
	*/
	...
	glBindBuffer(GL_ARRAY_BUFFER, vbo[7]);	// bufor wspolrzednych wierzcholkow sciany1
...
void configure_texture()
{
	...
	glGenTextures(1, &texw1);		// obiekt tekstury
	glBindTexture(GL_TEXTURE_2D, texw1);		// powiazanie tekstury z obiektem (wybor tekstury)
	...
	glGenTextures(1, &texw2);		// obiekt tekstury
	glBindTexture(GL_TEXTURE_2D, texw2);		// powiazanie tekstury z obiektem (wybor tekstury)
	...
	glGenTextures(1, &texw3);		// obiekt tekstury
	glBindTexture(GL_TEXTURE_2D, texw3);		// powiazanie tekstury z obiektem (wybor tekstury)
...
int main(int argc, char** argv)
{
	...
	while (true)
	{
		if (SDL_PollEvent(&windowEvent))
		{
			...
			if (windowEvent.type == SDL_KEYDOWN)
			{
				...
				case SDLK_UP:
					if (!(position.x + direction.x >= -4.5)) break;
					if (!((position.x + direction.x >= 3.5 || position.x + direction.x <= 2.5)) && (position.z + direction.z >= -3 && position.z +direction.z <= 2)) break;
					if (!((position.z + direction.z <= -3.5 || position.z + direction.z >= -2.5)) && (position.x + direction.x >= -2 && position.x + direction.x <= 3)) break;
						position.x += 0.5f * direction.x;
						position.z += 0.5f * direction.z;
					break;
				...
				direction.x = sin(2 * M_PI + alpha);
				direction.z = cos(2 * M_PI + alpha);
				tempx = position.x;
				tempz = position.z;
			}
		...
		translateMatrix = glm::translate(translateMatrix, glm::vec3(position.x, position.y, position.z)); // Przesunięcie trójkąta na pozycję kamery
		rotateMatrix = glm::rotate(rotateMatrix, alpha, glm::vec3(0.0f, 1.0f, 0.0f)); // Obrót trójkąta (jeśli alpha reprezentuje kąt obrotu)

		// Obliczenie modelMatrix z przesunięciem i obrotem trójkąta
		glm::mat4 modelMatrix = translateMatrix * rotateMatrix;

		// Utworzenie ostatecznej macierzy transformacji (projection * view * model)
		glm::mat4 transformMatrix = projectionMatrix * viewMatrix * modelMatrix;

		// Ustawienie transformacji w shaderze
		glUniformMatrix4fv(transformMatrixUniformLocation, 1, GL_FALSE, glm::value_ptr(transformMatrix));



		//transformMatrix = projectionMatrix * viewMatrix * rotateMatrix * translateMatrix;				// wynikowa macierz transformacji
		...
		glDrawElements(GL_TRIANGLES, 6, GL_UNSIGNED_INT, 0);	//rysujemy sciane1

		glBindVertexArray(vao[3]);
		glBindTexture(GL_TEXTURE_2D, texw2);
		glDrawElements(GL_TRIANGLES, 6, GL_UNSIGNED_INT, 0);	//rysujemy sciane2

		glBindVertexArray(vao[4]);
		glBindTexture(GL_TEXTURE_2D, texw3);
		glDrawElements(GL_TRIANGLES, 6, GL_UNSIGNED_INT, 0);	//rysujemy sciane3
	...
	glDeleteTextures(1, &texw1);
	glDeleteTextures(1, &texw2);
	glDeleteTextures(1, &texw3);
